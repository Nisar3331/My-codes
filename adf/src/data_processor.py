...
class CsvToParquetProcessor:
    ...
    Flow:
        ...
        3. Apply type inference and basic validation
        4. Convert to Parquet (Snappy compressed by default)
        5. Upload Parquet to landing container/folder
        6. Return a processing manifest
    """

    def __init__(self, connection_string: Optional[str] = None):
        conn_str = connection_string or AZURE_CONFIG.storage_connection_string
        if not conn_str:
            # Build from account name + key if connection string not provided
            conn_str = (
    ...
    def run(self) -> Dict[str, Any]:
        """
        Execute the full CSV → Parquet pipeline.
        ...
        if not csv_files:
            logger.warning("  No CSV files found in source path: %s/%s",
    ...
    def _list_csv_files(self):
        ...
        container: ContainerClient = self._client.get_container_client(self._source_container)
    ...
    def _process_single_file(self, blob_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            parquet_bytes = self._convert_to_parquet(df)

            logger.info("    Uploading: %s", parquet_blob_name)
            self._upload_blob(self._landing_container, parquet_blob_name, parquet_bytes)

            duration = (datetime.utcnow() - start).total_seconds()
            logger.info(
                "  Done: %s → %s (%.1fs, %d rows, %d cols)",
                blob_name, parquet_blob_name, duration, df.shape[0], df.shape[1]
            )
            return {
                "status": "success",
                "source": blob_name,
            ...
            logger.error("   Failed processing %s: %s", blob_name, str(exc))
            return {
                "status": "failed",
                "source": blob_name,
                "destination": parquet_blob_name,
    ...
    def _read_csv(self, csv_bytes: bytes) -> pd.DataFrame:
        """Read CSV bytes into a DataFrame with type inference."""
        df = pd.read_csv(
    ...
    def _convert_to_parquet(self, df: pd.DataFrame) -> bytes:
        """Serialise DataFrame to Parquet bytes."""
    ...
    def _infer_schema(self, df: pd.DataFrame) -> Dict[str, str]:
        """Return column → dtype mapping for the manifest."""
    ...
    def run_local(
        ...
        """
        Run processor on local filesystem — useful for CI / dev testing
        ...
        for fname in csv_files:
            ...
            try:
                ...
                duration = (datetime.utcnow() - start).total_seconds()
                logger.info("LOCAL  %s → %s (%.1fs, %d rows)", fname, dst_name, duration, len(df))
                manifest["files_processed"] += 1
